...
// Extensions Spark can natively parse. Used both for the file-picker's
// `accept` attribute and for a friendly pre-check before we hand bytes off
// to SplatMesh (Spark still does its own content-based detection - this is
// just to fail fast with a clear message on an obviously wrong file).
...
interface GaussianSplatViewerProps {
  /**
   * Optional splat/sog/ply URL to load on first mount, before the user has
   * picked a file of their own (relative to the `public` folder, e.g.
   * "/bonsai.spz"). Leave unset to start with just the file picker.
   */
  initialUrl?: string;
}
...
export default function SparkGaussianSplatViewer({
  initialUrl,
  ...
  // Populated by the three.js setup effect, read by the gesture handler and
  // the file-loading logic.
  const sceneRef = useRef<THREE.Scene | null>(null);
  const cameraRef = useRef<THREE.PerspectiveCamera | null>(null);
  const controlsRef = useRef<OrbitControls | null>(null);
  const sparkRendererRef = useRef<InstanceType<typeof SparkRenderer> | null>(
    null
  );
  const currentSplatRef = useRef<InstanceType<typeof SplatMesh> | null>(null);

  // Camera dolly range scales with whatever splat is currently loaded (a
  // tiny object and a city-scale scan need very different zoom limits), so
  // we keep it in a ref the gesture handler can read each frame.
  const distanceLimitsRef = useRef<{ min: number; max: number }>({
    min: 0.5,
    max: 80,
  });
  ...
  // Most Gaussian splat exports (.spz/.ply/.splat from COLMAP-style capture
  // pipelines) bake in a coordinate convention where "up" ends up flipped
  // relative to three.js's Y-up world, so they render upside-down unless
  // corrected with a 180deg rotation around X. Spark's own examples apply
  // this same fix. Default it on, but let the user toggle it off for files
  // that were already authored/exported in a Y-up convention.
  const [flipUpsideDown, setFlipUpsideDown] = useState(true);
  ...
  // --- Core three.js + Spark scene setup (runs once) ------------------------
  useEffect(() => {
    ...
    // Spark needs a SparkRenderer somewhere in the scene graph - it's what
    // actually sorts/rasterizes every SplatMesh's splats each frame. Once
    // it's added, calling THREE's own renderer.render(scene, camera) is all
    // that's needed; there's no manual per-frame splat.update() call like
    // some other splat libraries require.
    ...
    return () => {
      ...
      (currentSplatRef.current as unknown as { dispose?: () => void } | null)
        ?.dispose?.();
  ...
  // --- Frame the camera around whatever splat is currently loaded ----------
  const frameCameraToSplat = useCallback((mesh: InstanceType<typeof SplatMesh>) => {
    const camera = cameraRef.current;
    const controls = controlsRef.current;
    if (!camera || !controls) return;

    // getBoundingBox is available on SplatMesh; fall back to sane defaults
    // if it's ever unavailable (e.g. an older Spark version).
    const box: THREE.Box3 | undefined = mesh.getBoundingBox?.();
    ...
    if (box && !box.isEmpty()) {
      ...
      distanceLimitsRef.current = { min: 0.5, max: 80 };
  ...
  // --- Load a splat/sog/ply file (from <input> or drag-and-drop) -----------
  const loadSplatFile = useCallback(
    async (source: File | { url: string; name: string }) => {
      const scene = sceneRef.current;
      if (!scene) return;

      const fileName = "name" in source ? source.name : source.url;
      const ext = getExtension(fileName);
      if (ext && !SUPPORTED_EXTENSIONS.includes(ext as any)) {
        setSplatError(
          `Unsupported file type ".${ext}". Try one of: ${SUPPORTED_EXTENSIONS.join(
            ", "
          )}`
        );
        return;
      ...
      try {
        ...
        if (source instanceof File) {
          ...
          mesh = new SplatMesh({
            fileBytes,
            // Passing fileName lets Spark auto-detect the format from the
            // extension for types that can't be sniffed from content alone
            // (.splat / .ksplat), while still content-sniffing everything
            // else (.ply / .spz / .sog / .zip / .rad).
        ...
        // See the `flipUpsideDown` comment above - most splat files need
        // this 180deg correction around X to render right-side up.
        if (flipUpsideDown) {
        ...
        // Swap out whatever was loaded before.
        currentSplatRef.current?.removeFromParent();
        (currentSplatRef.current as unknown as { dispose?: () => void } | null)
          ?.dispose?.();
        ...
        setSplatError(
          err instanceof Error ? err.message : "Failed to load this file."
        );
        setIsLoadingSplat(false);
        setLoadProgress(null);
      }
    },
    [frameCameraToSplat, flipUpsideDown]
  );

  // Load the optional initial URL once, on mount (after the scene exists).
  useEffect(() => {
    if (!initialUrl) return;
    const name = initialUrl.split("/").pop() || initialUrl;
    void loadSplatFile({ url: initialUrl, name });
    // Intentionally only on mount - re-triggering on every render would
    // fight with user-picked files.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Let the user flip a currently-loaded splat without re-picking the file.
  useEffect(() => {
  ...
  const handleFileInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) void loadSplatFile(file);
    ...
    if (pinchA && pinchB) {
      ...
      if (memory.prevZoomDist !== null) {
        ...
        if (Math.abs(delta) > ZOOM_DEADZONE) {
          const { min, max } = distanceLimitsRef.current;
          zoomCamera(camera, controls, delta, min, max);
        }
      }
  ...
  return (
    ...
    >
      ...
      {!currentFileName && !isLoadingSplat && (
        <div
          style={{
            position: "absolute",
            inset: 0,
        ...
        >
          <div style={{ fontSize: 15, fontWeight: 600 }}>
            Drop a .splat, .sog, or .ply file here
          </div>
          <div style={{ fontSize: 13, opacity: 0.75 }}>
            (also supports .spz, .ksplat, .zip, .rad)
          </div>
        </div>
      )}
      ...
      >
        <div style={{ display: "flex", gap: 8 }}>
          ...
          >
            {currentFileName ? "Load different file\u2026" : "Choose file\u2026"}
          </button>

          <button
            onClick={() => setFlipUpsideDown((v) => !v)}
            title="Most splat files need this to render right-side up"
            style={{
              padding: "8px 14px",
              borderRadius: 8,
              border: "1px solid rgba(255,255,255,0.2)",
              background: flipUpsideDown
                ? "rgba(56, 189, 248, 0.9)"
                : "rgba(20, 20, 20, 0.75)",
              color: flipUpsideDown ? "#0a0a0a" : "#fff",
              fontSize: 14,
              fontWeight: 600,
              cursor: "pointer",
              backdropFilter: "blur(6px)",
          ...
          >
            Flip upright: {flipUpsideDown ? "On" : "Off"}
          </button>
